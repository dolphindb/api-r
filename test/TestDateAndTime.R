...
if (conn@connected == TRUE) {
    ...
    vec <- as.POSIXct(c("2018-08-14 16:16:16", "2018-08-17 03:26:56", "2018-08-25 19:24:46"))
    record <- assert(record, "test nanotimestamp vector", result, vec)

    result <- dbRun(conn, "2018.08.14T16:16:16.016023654")
    scalar <- as.POSIXct("2018-08-14 16:16:16")
    ...
    vec <- as.POSIXct(c("1970-01-01 16:16:16", "1970-01-01 16:16:17", "1970-01-01 16:16:18"))
    record <- assert(record, "test nanotime vector", result, vec)
    
    result <- dbRun(conn, "16:16:16.016265874")
    scalar <- as.POSIXct("1970-01-01 16:16:16")
    ...
    vec <- as.POSIXct(c("1970-01-01 16:16:16", "1970-01-01 16:17:16", "1970-01-01 16:18:16"))
    ...
    vec <- as.POSIXct(c("1970-01-01 16:16:00", "1970-01-01 16:17:00", "1970-01-01 16:18:00"))
    record <- assert(record, "test minute vector", result, vec)
    
    result <- dbRun(conn, "16:16m")
    scalar <- as.POSIXct("1970-01-01 16:16:00")
    ...
    vec <- as.POSIXct(c("2018-07-06 00:01:01", NA, "2018-09-30 03:29:56"))
    ...
    scalar <- as.POSIXct("2018-07-06 12:01:01")
